...
while true; do
    read -p "This will create a New Home Assistant Core LXC. Proceed(y/n)?" yn
    case $yn in
        [Yy]* ) break;;
        [Nn]* ) exit;;
...
wget -qL https://raw.githubusercontent.com/tteck/Proxmox/main/core_setup.sh
...
pct create $CTID $TEMPLATE_STRING -arch $ARCH -features nesting=1 \
  -hostname $HOSTNAME -net0 name=eth0,bridge=vmbr0,ip=dhcp -onboot 1 -cores 2 -memory 4096 \
...
pct start $CTID
pct push $CTID core_setup.sh /core_setup.sh -perms 755
pct exec $CTID /core_setup.sh

# Get network details and show completion message
IP=$(pct exec $CTID ip a s dev eth0 | sed -n '/inet / s/\// /p' | awk '{print $2}')
info "Successfully created Home Assistant Core LXC to $CTID."
msg "
