#!/usr/bin/env bash
while true; do
    read -p "Use to copy all data from one Nginx Proxy Manager LXC to another. Proceed(y/n)?" yn
    case $yn in
        [Yy]* ) break;;
        [Nn]* ) exit;;
...
TITLE="Nginx Proxy Manager LXC Data Copy"
...
Version: 2022.01.21" 13 50 || exit
info "Nginx Proxy Manager Data from '$CTID_FROM' to '$CTID_TO'"
...
[ -d "${CTID_FROM_PATH}${DATA_PATH}" ] || \
  die "Nginx Proxy Manager directories in '$CTID_FROM' not found."
...
# Use to copy all data from one Nginx Proxy Manager LXC to another
# run from the Proxmox Shell
# bash -c "$(wget -qLO - https://raw.githubusercontent.com/tteck/Proxmox/main/misc/npm-copy-data.sh)"