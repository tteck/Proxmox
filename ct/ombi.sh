...
function default_settings() {
        ...
        HN=$NSAPP
        echo -e "${DGN}Using Disk Size ${BGN}5${CL}${DGN}GB${CL}"
        DISK_SIZE="5"
...
header_info
        ...
        echo -e "${YW}Enter a Disk Size, or Press [ENTER] for Default: 5 "
        read DISK_SIZE
        if [ -z $DISK_SIZE ]; then DISK_SIZE="5"; fi;
...
export PCT_OPTIONS="
  ...
  $PW
"
bash -c "$(wget -qLO - https://raw.githubusercontent.com/Kickbut101/Proxmox/main/ct/create_lxc.sh)" || exit

msg_info "Starting LXC Container"
pct start $CTID
msg_ok "Started LXC Container"

lxc-attach -n $CTID -- bash -c "$(wget -qLO - https://raw.githubusercontent.com/Kickbut101/Proxmox/main/setup/ombi-install.sh)" || exit

IP=$(pct exec $CTID ip a s dev eth0 | sed -n '/inet / s/\// /p' | awk '{print $2}')

pct set $CTID -description "# ${APP} LXC
### https://github.com/Kickbut101/Proxmox"
