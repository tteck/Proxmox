...
while true; do
    read -p "This will create a New Agent DVR LXC Container. Proceed(y/n)?" yn
    case $yn in
        [Yy]* ) break;;
        [Nn]* ) exit;;
...
wget -qL https://raw.githubusercontent.com/tteck/Proxmox/main/setup/agent_setup.sh
...
pct start $CTID
pct push $CTID agent_setup.sh /agent_setup.sh -perms 755
pct exec $CTID /agent_setup.sh

IP=$(pct exec $CTID ip a s dev eth0 | sed -n '/inet / s/\// /p' | awk '{print $2}')
info "Successfully Created Agent DVR LXC to $CTID."
echo -e "\e[1;92m Agent DVR should be reachable by going to the following URL.
             http://${IP}:8090
\e[0m"